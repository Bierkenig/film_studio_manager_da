  ...
  methods: {
    async startGame() {
      ...
      if (this.databaseType === 'current') {
        process.env.NODE_ENV !== 'production' ? window.ipcRenderer.send('changeDBPath', "public/DB/fsm_custom" + this.databaseVersion + ".db") : window.ipcRenderer.send('changeDBPath', "windows/bundled/DB/fsm_custom" + this.databaseVersion + ".db")
      } else {
        process.env.NODE_ENV !== 'production' ? window.ipcRenderer.send('changeDBPath', "public/DB/database/fsm.db") : window.ipcRenderer.send('changeDBPath', "windows/bundled/DB/database/fsm.db")
      }
      //Fetch Topics
      let allTopics = []
