  ...
  <background-tile class="projectTile" :title="$t('projects')">
    <tile-pages-nav id="projectsNavigation" :pages='[all,screenplaysText,productionsText]' :gradient='true'>
      <div class="projectsSectionElement verticalScroll">
        <screenplay-element class="projectsElement" v-for="(it, index) in this.screenplays" :key="index" :screenplay-title="it.title" :age="RegExp('\\+\\d+$').exec(it.ageRating)[0]" :genre="it.genre.genreName" :genre-icon="it.genre.genreName.toLowerCase()" :quality="it.rating" :writer="it.writer._first_name + ' ' + it.writer._last_name" @open-clicked="screenplayInfo(it)"/>
        <project-element class="projectsElement" v-for="(it, index) in this.productions" :key="index" :project-title="it._preProduction.screenplay.title" :age="RegExp('\\+\\d+$').exec(it._preProduction.screenplay.ageRating)[0]" :genre="it._preProduction.screenplay.genre.genreName" :genre-icon="it._preProduction.screenplay.genre.genreName.toLowerCase()" :release="it._preProduction.releaseDate.toLocaleDateString('de-DE')" :status="it._status" @open-clicked="movieInfo(it)"/>
      </div>
      <div class="projectsSectionElement verticalScroll">
        <screenplay-element class="projectsElement" v-for="(it, index) in this.screenplays" :key="index" :screenplay-title="it.title" :age="RegExp('\\+\\d+$').exec(it.ageRating)[0]" :genre="it.genre.genreName" :genre-icon="it.genre.genreName.toLowerCase()" :quality="it.rating" :writer="it.writer._first_name + ' ' + it.writer._last_name" @open-clicked="screenplayInfo(it)"/>
      </div>
...
export default {
  ...
  watch: {
    '$store.getters.getInProductionMovies.length': function (){
      this.productions = this.$store.getters.getInProductionMovies;
    }
  },
