    ...
    <div class="modal-wrapper">
      <div class="modal-container">
        <div class="modal-body">
          <slot name="body">
            <div>{{$t('productionNotFinished.fin')}}</div>
            <div v-if="cause === 1">{{$t('productionNotFinished.cause1')}}</div>
            <div v-if="cause === 2">{{$t('productionNotFinished.cause2')}}</div>
            <div v-if="cause === 3">{{$t('productionNotFinished.cause3')}}</div>

            <div>{{$t('productionNotFinished.options')}}</div>
            <div>{{$t('productionNotFinished.option1')}}</div>
            <div>{{$t('productionNotFinished.option2')}}</div>

            <button class="modal-default-button" @click="this.$store.state.currentMovie.quality *= 0.85; $emit('close')">{{$t('productionNotFinished.stop')}}</button>
            <button class="modal-default-button" @click="resumeProd(); $emit('close')">{{$t('productionNotFinished.resume')}}</button>
          </slot>
        </div>
      </div>
    </div>
